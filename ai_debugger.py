...
Recursively scans all .py files in the project, sends each to Groq
for static analysis (async bugs, missing deps, Pydantic/SQLAlchemy
...
DEFAULT_CONCURRENCY = 5   # simultaneous Groq requests
...
# ------------------------------------------------------------------
# Groq call (with retry + concurrency control)
# ------------------------------------------------------------------
...
async def analyze_file(
    ...
    async with semaphore:
        ...
        return FileResult(
            path=rel_path,
            status="error",
            error_message=f"Groq API failed after {API_RETRY_ATTEMPTS} attempts: {last_error}",
        )
...
async def main_async(args: argparse.Namespace) -> int:
    ...
    tasks = [analyze_file(client, f, root, semaphore) for f in files]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="AI-powered Python repo debugger using Groq.")
    ...
    parser.add_argument(
        ...
        help=f"Max concurrent Groq requests (default: {DEFAULT_CONCURRENCY})",
    )
