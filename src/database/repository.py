...
from src.database.models import User, Chat, ConversationHistory, Project, GeneratedFile, UsageStatistic, LinkAsset
...
# LinkRepository - Owner ke shared download links store aur search karne ke liye
class LinkRepository(BaseRepository):
    async def add_link(self, name: str, url: str, added_by: int, note: Optional[str] = None) -> LinkAsset:
        link = LinkAsset(name=name.strip(), url=url.strip(), added_by=added_by, note=note)
    ...
    async def search(self, query: str, limit: int = 5) -> List[LinkAsset]:
        ...
        stmt = (
            select(LinkAsset)
            .where(LinkAsset.name.ilike(f"%{clean_query}%"))
            .order_by(LinkAsset.created_at.desc())
            .limit(limit)
        )
        result = await self.session.execute(stmt)
        return list(result.scalars().all())

    async def list_recent(self, limit: int = 20) -> List[LinkAsset]:
        stmt = select(LinkAsset).order_by(LinkAsset.created_at.desc()).limit(limit)
        result = await self.session.execute(stmt)
        return list(result.scalars().all())

    async def delete_by_id(self, link_id: int) -> bool:
        stmt = delete(LinkAsset).where(LinkAsset.id == link_id)
        result = await self.session.execute(stmt)
